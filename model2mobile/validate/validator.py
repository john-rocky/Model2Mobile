...
def run_validation(
    ...
    try:
        ...
        if task == "detect":
            ...
            checks.extend(validate_detection(pt_outs, cm_outs, pairs, config))
        elif task == "classify":
            from model2mobile.validate.task_classify import validate_classification
            checks.extend(validate_classification(pt_outs, cm_outs, pairs, config))
        elif task == "segment":
            from model2mobile.validate.task_segment import validate_segmentation
            checks.extend(validate_segmentation(pt_outs, cm_outs, pairs, config))
        elif task == "depth":
            from model2mobile.validate.task_depth import validate_depth
            checks.extend(validate_depth(pt_outs, cm_outs, pairs, config))

        # General tensor comparison as extra signal (all tasks)
